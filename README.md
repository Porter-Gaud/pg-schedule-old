...
*Hint: The system of seperating model code/data, view code, and the controller code that links them is a very common style of creating apps known as [MVC](https://en.wikipedia.org/wiki/Model%E2%80%93view%E2%80%93controller), and is worth a look into if you are interested in software development.*

##### Manage
Manage is mainly designed to be a way for administrators at Porter-Gaud to upload alternate schedules.
...
If you change something Passport/Postgres/Google Auth related, please ensure that you test the changes in production mode before pushing (the pre-push pipeline will do additional testing).
...
~~~~
Additionally, it is worth noting that the server will decide that is is producton if the `PORT` env variable is set to 80.
...
If using the official Google credentials, this will work immidiately.  Otherwise, you will have to set up the OAuth credentials in Google Cloud on your own.
...
~~~~
Replace ``[BRANCH-NAME]`` with your actual issue branch's name.  For example, if I were rebasin into the branch of Issue #555 from Step 4, I would type `git rebase 555`
...
**When I try to run the server, it immidiately crashes!**
...
If you are unsure on how to contribute to this app effectively or what some code means, don't hesistate to ask a member of the project team.
